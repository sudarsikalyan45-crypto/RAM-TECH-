...
object GeminiClient {
    ...
    suspend fun generateContent(prompt: String, systemInstruction: String? = null): String = withContext(Dispatchers.IO) {
        ...
        if (apiKey.isEmpty() || apiKey == "MY_GEMINI_API_KEY") {
            ...
            return@withContext "API key is not configured. Please add your GEMINI_API_KEY to the secrets panel in AI Studio."
        }
        ...
        try {
            ...
            client.newCall(request).execute().use { response ->
                if (!response.isSuccessful) {
                    ...
                    return@withContext "Error: Request failed with status ${response.code}. Please ensure your API Key is valid."
                }
                ...
                if (candidates != null && candidates.length() > 0) {
                    ...
                    if (parts != null && parts.length() > 0) {
                        return@withContext parts.getJSONObject(0).optString("text", "No response.")
                    }
                }
